...
exports.router.post('/topup', async (req, res) => {
    ...
    try {
        ...
        // เติมเงิน
        await DBconnect_1.conn.query("UPDATE user SET money = money + ? WHERE uid = ?", [amount, uid]);
        // ดึงยอดเงินใหม่
        const [updatedUsers] = await DBconnect_1.conn.query("SELECT money FROM user WHERE uid = ?", [uid]);
